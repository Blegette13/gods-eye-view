...
test('BDP layer registry adds parcel, flood, and RRC layers with unique identities', () => {
  ...
  assert.equal(BDP_LAYER_STATE_REGISTRY.length, 3);
  assert.equal(new Set(BDP_LAYER_STATE_REGISTRY.map((entry) => entry.id)).size, 3);
  assert.equal(new Set(BDP_LAYER_STATE_REGISTRY.map((entry) => entry.token)).size, 3);
});
